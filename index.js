...
io.on('connection', (socket) => {
    ...
    socket.on("message", (data) => {
        console.log("Message reçu de l'ESP32 :", data);
        //console.log("Message reçu de l'ESP32 :", players[data-1]);
        io.emit("winner", data);
    });   
    
    socket.on("anim", (data) => {      
        console.log("Message reçu de anim:", data);  
        //io.emit("winner", { player: data });
        if(data==="next"){
            console.log("next")
            question = getRandomQuestion();
            io.emit("question", question);   
    ...
    // Exemple : envoyer un message à l’ESP32
