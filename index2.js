...
io.on('connection', (socket) => {
	...
	// --- Enregistrement du type de client ---
	socket.on("register", (type) => {
		socket.isESP32 = (type === "esp32");
		console.log("Client identifié :", socket.id, "ESP32 =", socket.isESP32);
	});

	// --- BUZZER (uniquement ESP32) ---
	socket.on("buzz", (data) => {
		
		console.log("Buzz reçu :", data); 
		//console.log("Buzz reçu :", socket); 
		//if (!socket.isESP32) return; // ignore navigateur
	...
	// --- ANIM depuis navigateur ---
	socket.on("anim", (data) => {
		console.log("Message anim :", data);
